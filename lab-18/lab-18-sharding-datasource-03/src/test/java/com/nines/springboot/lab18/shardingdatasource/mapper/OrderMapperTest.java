...
class OrderMapperTest {
    ...
    public void testSelectById() { // 测试从库的负载均衡
        for (int i = 0; i < 4; i++) {
            OrderDO order = orderMapper.selectOne(new LambdaQueryWrapper<OrderDO>().eq(OrderDO::getUserId, 15));
    ...
    public void testSelectByUserId() {
        // 测试读操作
        OrderDO order = orderMapper.selectOne(new LambdaQueryWrapper<OrderDO>().eq(OrderDO::getUserId, 2));
        System.out.println(order);
    }
